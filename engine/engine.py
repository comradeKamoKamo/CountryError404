...
def save_to_s3(s3, bucket):
    ...
    if Path("tmp/protected_users.pickle").exists():
        bucket.upload_file('tmp/protected_users.pickle',
                           'protected_users.pickle')
    return
...
def save_from_s3(s3_name):
    ...
    try:
        ...
        bucket.download_file("protected_users.pickle",
                             "tmp/protected_users.pickle")
    except:
...
def tweet_news(api):
    ...
    for tweet in api.list_timeline(list_id="1059060887087017985", tweet_mode="extended", count=900):
